...
export const ChatTab: React.FC<Props> = ({ apiClient, tenantId, userId }) => {
  ...
  const [journey, setJourney] = useState<JourneyState>({ stage: 'ask_name', assistant_mode: 'sandbox', consent: false });
  ...
  const loadSession = async (id: string) => {
    ...
    try {
      ...
      setJourney(data.journey || { stage: 'ask_name' });
  ...
  const newChat = async () => {
    ...
    try {
      ...
      setMessages([]); setJourney(session.journey || { stage: 'ask_name', assistant_mode: 'sandbox', consent: false }); setToolEvents([]);
  ...
  const send = async (text: string, actions: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      setInput('');
      const response = await apiClient.sendMessage(id, text, actions);
  ...
  const uploadCv = async (file?: File) => {
    if (!file) return;
    if (!journey.linked_entity_id) { setError('Please share your name first so Anya can link the CV to this sandbox conversation.'); return; }
  ...
  const renderArtifact = (artifact: ChatArtifact, key: string) => {
    if (artifact.type === 'vacancies') return (
      <div key={key} className="grid gap-3 sm:grid-cols-2">
        {(artifact.items || []).map((job: any) => {
          const selected = Number(journey.selected_vacancy_id) === Number(job.id);
          const selectionLabel = selected && journey.interest_status === 'recorded' ? 'Interest recorded' : selected ? 'Selected' : 'I’m interested';

          return <article key={job.id} className={`rounded-2xl border bg-white p-4 shadow-sm ${selected ? 'border-emerald-300 ring-1 ring-emerald-100' : 'border-teal-200'}`}>
            ...
            <button disabled={busy || selected} onClick={() => void send(`I'm interested in ${job.title} (${job.reference || job.id}).`, { selected_vacancy_id: job.id })} className={`mt-4 w-full rounded-xl px-3 py-2 text-sm font-semibold text-white ${selected ? 'cursor-default bg-emerald-600' : 'bg-teal-600 hover:bg-teal-700'}`}>{selectionLabel}</button>
          </article>;
        })}
      </div>
    );
    if (artifact.type === 'consent_request') return (
      <div key={key} className="rounded-2xl border border-teal-200 bg-teal-50 p-4 text-slate-800">
        <div className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4 text-teal-600" />{artifact.title || 'Save your details with JobBazaar?'}</div>
        <p className="mt-2 text-sm text-slate-600">{artifact.body}</p>
        <div className="mt-3 flex flex-wrap gap-2">
          <button disabled={busy || journey.consent === true} onClick={() => void send('Yes, you may save my recruitment details.', { consent_granted: true })} className="rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{journey.consent ? 'Allowed' : artifact.accept_label || 'Allow and continue'}</button>
          {!journey.consent && <button disabled={busy} onClick={() => void send('Not now. I’d like to keep chatting without saving my details.', { consent_declined: true })} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-600">{artifact.decline_label || 'Not now'}</button>}
        </div>
      </div>
    );
    if (artifact.type === 'profile_review') return (
      <div key={key} className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-slate-800">
        <div className="flex items-center gap-2 font-semibold"><FileText className="h-4 w-4 text-amber-600" />Profile review</div>
        ...
        {Object.keys(artifact.fields || {}).length > 0 && <button onClick={() => void send('I reviewed these profile details and confirm they are correct.', { confirmed_fields: Object.keys(artifact.fields || {}), profile_fields: artifact.fields })} className="mt-3 rounded-xl bg-amber-600 px-4 py-2 text-sm font-semibold text-white">Confirm these details</button>}
      </div>
    );
  ...
  return <div className="mx-auto flex h-full w-full max-w-[1500px] gap-4 overflow-hidden p-3 sm:p-5">
    ...
    <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-3xl border border-slate-800 bg-[#f4f7f7] shadow-2xl">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        {inspector && <aside className="hidden w-80 shrink-0 overflow-y-auto border-l border-slate-200 bg-slate-950 p-4 text-xs text-slate-300 md:block"><h3 className="mb-4 flex items-center gap-2 font-bold text-white"><ShieldCheck className="h-4 w-4 text-violet-400" />Sandbox inspector</h3><InspectorRow label="Journey" value={journey.stage} /><InspectorRow label="Interest" value={journey.interest_status || 'none'} /><InspectorRow label="Linked record" value={journey.linked_entity_type ? `${journey.linked_entity_type} #${journey.linked_entity_id}` : 'Not linked'} /><InspectorRow label="Consent" value={journey.consent_status === 'not_required' ? 'Not required (sandbox)' : journey.consent ? 'Recorded' : 'Not recorded'} /><InspectorRow label="CV" value={journey.cv_status} /><InspectorRow label="Ready for review" value={journey.review_ready ? 'Yes' : 'No'} /><InspectorRow label="Confirmed fields" value={(journey.confirmed_fields || []).join(', ') || 'None'} /><InspectorRow label="Conversion" value={journey.conversion_status || 'Not requested'} /><InspectorRow label="Appointment" value={journey.appointment_status || 'Not booked'} /><h4 className="mb-2 mt-5 font-bold text-slate-100">Tool traces</h4><div className="space-y-2">{toolEvents.length ? toolEvents.map((event, index) => <div key={index} className="rounded-lg border border-slate-800 bg-slate-900 p-2"><div className="flex justify-between"><span>{event.tool}</span><span className={event.status === 'success' ? 'text-emerald-400' : 'text-red-400'}>{event.status}</span></div><span className="text-slate-600">{event.latency_ms} ms · arguments redacted</span></div>) : <p className="text-slate-600">No tools called yet.</p>}</div>{journey.assistant_mode === 'sandbox' && journey.linked_entity_id && <div className="mt-5 rounded-xl border border-slate-800 p-3"><UserRoundCheck className="mb-2 h-4 w-4 text-teal-400" /><p>This is a tagged, phone-free sandbox record. It is intentionally retained for staff review.</p></div>}
          </aside>}
      </div>
    </section>
  </div>;
